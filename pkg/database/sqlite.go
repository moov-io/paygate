...
var (
	...
	sqliteMigrations = migrator.Migrations(
		execsql(
			"create_tenants",
			`create table tenants(tenant_id primary key, name, primary_customer, company_identification, created_at datetime, deleted_at datetime);`,
		),
		execsql(
			"create_organizations",
			`create table organizations(organization_id primary key, tenant_id, name, primary_customer, created_at datetime, deleted_at datetime);`,
		),
		execsql(
		...
		execsql(
			"create_transfers",
			`create table if not exists transfers(transfer_id primary key, tenant_id, amount, source_customer_id, source_account_id, destination_customer_id, destination_account_id, description, status, same_day, return_code, created_at datetime, last_updated_at datetime, deleted_at datetime);`,
		),
		execsql(
