...
func createMicroDeposits(
	...
	if xfer, err := originate(cfg, tenantID, companyIdentification, amt1, src, dest, repo, strategy, pub); err != nil {
		return nil, err
	} else {
		micro.TransferIDs = append(micro.TransferIDs, xfer.TransferID)
	}
	if xfer, err := originate(cfg, tenantID, companyIdentification, amt2, src, dest, repo, strategy, pub); err != nil {
		return nil, err
	} else {
		micro.TransferIDs = append(micro.TransferIDs, xfer.TransferID)
	}
	...
	if xfer, err := originate(cfg, tenantID, companyIdentification, sum.String(), src, dest, repo, strategy, pub); err != nil {
		return micro, err
	} else {
...
func originate(
	...
	if err := transferRepo.WriteUserTransfer(tenantID, xfer); err != nil {
		return nil, err
	}
